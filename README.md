...
JDBC ledger – debit the source account and credit the destination.
SQLite is the demo default (`FASTPAY_DB`); Postgres is the HA option
(`FASTPAY_JDBC_URL=jdbc:postgresql://…`). The store class is `Ledger`
(formerly `InMemoryLedger`).
...
CI – GitHub Actions runs `./gradlew test` (including Postgres) and `docker build`
...
step (not a multi-region payments fabric), the TLS certs are localhost
self-signed, and API keys are hashed bearer tokens—not a bank-grade IAM or PCI program.
